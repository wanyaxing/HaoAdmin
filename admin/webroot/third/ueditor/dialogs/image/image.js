        ...
        initUploader: function () {
            ...
            function addFile(file) {
                ...
                $btns.on('click', 'span', function () {
                    ...
                    switch (index) {
                        case 0:
                            if (file.getStatus()=='invalid' || file.getStatus()=='cancelled'){removeFile(file);return ;}//axing add
                            uploader.removeFile(file);
                            return;
                        case 1:
                            file.rotation += 90;
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                var $file = $('#' + file.id);
                try {
                    var responseText = (file.ret || ret._raw || ret ),//axing edit
            ...
            uploader.on('error', function (code, max, file) {//axing edit
                if (code == 'Q_TYPE_DENIED' || code == 'F_EXCEED_SIZE') {
                    addFile(file);
                }
                setState('ready', file);//axing add
                // setState('finish', file);//axing add
            });
